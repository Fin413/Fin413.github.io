...
/*
    ...
    can remove src folder when uploading to github cause it's not used
...
function checkForMatch(){
    ...
    alreadyGuessed.forEach(name => {
        if(name == value) alreadyGuessedIndicator();
    });
    ...
    for (const property in data) {
        ...
        if(name.includes(value)){
            ...
            data[property].forEach(temp => alreadyGuessed.push(temp) );
            delete data[property];

            indicator.classList.remove("alreadyGuessed");
...
function alreadyGuessedIndicator() {
    if(indicator.classList.contains("alreadyGuessed")) return;

    input.style.animation = "shakeRegion 200ms linear 3";
    ...
    setTimeout(() => {
        ...
        indicator.innerText = "Already Guessed";
        indicator.classList.remove("indicatorHide");
        indicator.classList += " alreadyGuessed";
    }, 600);
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            indicator.classList += " indicatorHide";
            indicator.classList.remove("alreadyGuessed");
