...
function setBackgroundValues(){
    ...
    canvasPositions[0] = {x: rect.left + offset, y: rect.top + offset};
    canvasPositions[1] = {x: rect.right - offset, y: rect.top + offset};
    canvasPositions[2] = {x: rect.right - offset, y: rect.bottom - offset};
    canvasPositions[3] = {x: rect.left + offset, y: rect.bottom - offset};


    if(window.innerWidth > 680){
        leftBound = document.getElementById("optionsContainer").getBoundingClientRect().left;
        bottomBound = window.innerHeight;
    } else {
        bottomBound = document.getElementById("optionsContainer").getBoundingClientRect().top;
