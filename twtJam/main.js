...
/*
    ...
    responsuve shit especially the game side menu
    ...
    does timeout work when computer is closed
    cause if not my whole thing is fucked

    add ability to retake tutorial in settings
    ...
    - tutorial
    - side game bar resizing still fucky
    ...
    - make weight transformation OBIOUS after working out
    ...
    - make main page words better
    - edit main page scroll <- !!
...
function readData() {
    ...
    if(data.lastWorkout != null){
        ...
        document.getElementById("workoutNotifContainer").style.bottom = 0;
    }
...
function initData(){
    ...
    // if you visit index.html without params and no existing data 
...
window.addEventListener("beforeunload", (e) => {
    // saves data when user reloads or exits page
    saveData();
});
...
function calcAge() {
    ...
    let count = Math.round(diff / ( 8.64 * Math.pow(10, 7))); // for hours -> ( 3.6 * Math.pow(10, 6)));
    let unitOfTime = "days";
...
function calcChange() {
    ...
    console.log("it's been " + hours + " seconds since your last visit.")
    console.log("You hunger went up by " + hours);
    console.log("You weight went up by " + Math.round(hours/2));
}


// make these smaller increments so that it updates with smaller values but less frequently 
var blah = 0;
setInterval(() => {
    // for every hour on the page increase hunger by one
    if(alienHunger < 10) alienHunger +=  10 / 12; // 3600000 / 300000 = 12
    setHunger();

    blah += 5;
    console.log("It's been " + blah + " minutes.");
    if(blah == 60) console.log("ONE HOUR HUNGER SHOULD HAVE GONE DOWN ONE");
    console.log("Hunger is now: " + alienHunger);

}, 300000); // 3600000

setInterval(() => {
    // for every two hours on the page increase size by one
    increaseSize(1);
}, 7200000);

function hideNotif(){
    document.getElementById("workoutNotifContainer").style.bottom = "-1000px";
}

function showMoreInfo() {
    document.getElementById("lastWorkout").innerText += " You gained " + + " weight";
}



let transitionEl = document.getElementById("transitionEl");
...
function startTutorial() {
    ...
    if(document.getElementById("settingsContainer").style.left == "0px") toggleSettingsDrawer();
...
var tutorialOverlay = document.getElementsByClassName("tutorialOverlay")[0];
var gameBtns = document.getElementsByClassName("gameBtn");
// initialize these in the start tutorial??? so if you dont do the tutorial theyre not loaded
        ...
        if(sections[i].style.display == "flex"){
            ...
            switch(page){
                ...
                case 4:
                    ...
                    sections[page].firstElementChild.style.top =  (gameBtns[0].getBoundingClientRect().top - 100) + "px";
                    
                    gameBtns[0].onclick = () =>{
                        nextSection();
                        showExercise();
                    }
                    gameBtns[1].disabled = true;
                    break;
                case 5:
                    ...
                    break;
                case 6:
                    gameBtns[0].onclick = showExercise;
                    gameBtns[0].disabled = true;
                    gameBtns[1].disabled = false;
                    gameBtns[1].onclick = () =>{
                        startGame();
                        nextSection();
                    }
                    ...
                    let sectionText = sections[page].firstElementChild;
                    sectionText.innerText = "Pressing the \"Eat\" button will fill up your hunger bar. Try clicking it now";
                    sectionText.style.position = "absolute";
                    sectionText.style.top =  (gameBtns[0].getBoundingClientRect().top - 100) + "px";
                    break;
                case 7:
                    gameBtns[0].disabled = false;
                    gameBtns[1].onclick = startGame;
...
function showTutorialBtns(){
    let topClip = (gameBtns[0].getBoundingClientRect().top - 10) + "px"; //((gameBtns[0].getBoundingClientRect().top - 10) / window.innerHeight) * 100 + "%";
    let bottomClip = (gameBtns[0].getBoundingClientRect().bottom + 10) + "px";
...
let img = document.getElementById("exerciseGraphic");
var imgInterval;
...
function showExercise() {
    ...
    setTimer(1);//exercise.time);

    img.src = "media/exercises/" + exercise.name + "1.svg";

    imgInterval = setInterval(()=>{
        img.src = "media/exercises/" + exercise.name + "2.svg";
        setTimeout(()=>{
            img.src = "media/exercises/" + exercise.name + "1.svg";
        }, 500);
    }, 1000);
...
function hideExercise() {
    ...
    console.log(exerciseElement.style.visibility);
...
var speed; // if you change this change increase size too
...
function resize(){
    ...
    // fix letter spacing on resize

    //3/2 ratio
    // canvas.height = document.documentElement.clientHeight - 30;
    // canvas.width = canvas.getBoundingClientRect().height * ( 3/2 );
    // ctx.scale(canvas.width/1000, canvas.height/666);

    // alienSize = canvas.width/10;
    ...
    // if(ratio == undefined) ratio = [canvas.width, canvas.height];

    // ctx.scale(canvas.width/ratio[0], canvas.height/ratio[1]);
}
...
// var sizeInterval = setInterval(increaseSize, 500);
var spawnFood;
...
window.onfocus = () => {
    ...
    if(timeElapsed > 300000){
        ...
        // let properWeight = dataOnBlur.divisor + ()
...
function setHunger() {
    ...
    let color = pickHex(percent/100);
    
    hungerElement.style.backgroundColor = "rgb(" + color[0] + "," + color[1] + ", 0";
...
var hungerElement = document.getElementById("hungerContainer");

function toggleGamePanel() {
    let gamePanel = document.getElementById("gameBtns");
...
function startGame() {
    ...
    if(divisor <= 6){
        ...
        document.getElementById("workoutNotifContainer").style.backgroundColor = "red";
...
/*
    SHIT LIST
    for excercises add "Don't know how to do this?" links
    for videos and articles on how to do the exercises
    include actual health tips and advice
    like a page to tell people how to get started and resources
*/
