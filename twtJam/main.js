...
/*
    ...
    if you exercise then go into setting and click "retake tutorial" you can skip the timer

    LETTER SPACING IN CANVAS
    add onload so elements only start when everything is loaded
    exercise overlay goes over settings BAD
    after weight shit
    tutorial ends weird
...
function readData() {
    ...
    if(data.lastWorkout != null){
        ...
        document.getElementById("workoutNotifContainer").style.visibility = "visible";
    }
...
function hideNotif(){
    ...
    document.getElementById("workoutNotifContainer").style.visibility = "hidden";
}
...
function startTutorial() {
    ...
    if(document.getElementById("exercise").style.visibility == "visible") hideExercise();
...
var actionBtns = document.getElementsByClassName("actionBtn");
...
function showTutorialBtns(){
    let topClip = (actionBtns[0].getBoundingClientRect().top - 10) + "px"; //((gameBtns[0].getBoundingClientRect().top - 10) / window.innerHeight) * 100 + "%";
